...
def read_json_ids(fstream): 
    # fstream = .json filename 
...
def read_json(fstream): 
    ...
    for k in range(len(context)-1):
        ...
        #print(answer[k].split(' '))
        ans_intList = [int(val) for val in answer[k].split(' ')]
        dataset.append((question[k].split(' '),context[k].split(' '),ans_intList))
    #print(dataset[0])
    return dataset
